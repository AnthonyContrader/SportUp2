...
namespace SportUp.Controllers
{
    ...
    public class ValuesController : Controller
    {
        ...
        public async Task<IEnumerable<string>> Get()
        {
            ...
            {
                var queryResult = await client.KV.List("SportUp-ID-");
        ...
        public async Task<string> Get(int id)
        {
            using (var client = _consulClientFactory())
            {
                var getPair = await client.KV.Get($"SportUp-ID-{id.ToString()}");
        ...
        public async Task Put(int id, [FromBody]JObject jsonData)
        {
            using (var client = _consulClientFactory())
            {
                ...
                var putPair = new KVPair($"SportUp-ID-{id.ToString()}")
                {
        ...
        public async Task Delete(int id)
        {
            using (var client = _consulClientFactory())
            {
                await client.KV.Delete($"SportUp-ID-{id.ToString()}");
            }
        }
    }
}
